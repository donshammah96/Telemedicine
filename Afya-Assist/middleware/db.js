...
const saveDataToDatabase = async (deviceId, data) => {
    const query = 'INSERT INTO IoTData (deviceId, data) VALUES (?, ?)';
    await connection.execute(query, [deviceId, JSON.stringify(data)]);
};

const getIoTData = async () => {
    const query = 'SELECT timestamp, data FROM IoTData ORDER BY timestamp DESC LIMIT 100';
